...
patchworkpp::Nodelet<PointT>::Nodelet()
{
}

template<typename PointT>
void
patchworkpp::Nodelet<PointT>::onInit()
{
  // Init ROS related
  ...
  int num_polygons =
      std::inner_product(num_rings_each_zone_.begin(), num_rings_each_zone_.end(), num_sectors_each_zone_.begin(), 0);
  ...
  PlaneViz = node_handle_.advertise<jsk_recognition_msgs::PolygonArray>("plane", 10, true);
  pub_revert_pc = node_handle_.advertise<sensor_msgs::PointCloud2>("revert_pc", 10, true);
  pub_reject_pc = node_handle_.advertise<sensor_msgs::PointCloud2>("reject_pc", 10, true);
  pub_normal = node_handle_.advertise<sensor_msgs::PointCloud2>("normals", 10, true);
  pub_noise = node_handle_.advertise<sensor_msgs::PointCloud2>("noise", 10, true);
  pub_vertical = node_handle_.advertise<sensor_msgs::PointCloud2>("vertical", 100, true);
  pub_cloud = node_handle_.advertise<sensor_msgs::PointCloud2>("cloud", 10, true);
  pub_ground = node_handle_.advertise<sensor_msgs::PointCloud2>("ground", 10, true);
  pub_non_ground = node_handle_.advertise<sensor_msgs::PointCloud2>("nonground", 10, true);
  ...
  node_handle_.param<string>("cloud_topic", cloud_topic_, "/pointcloud");
  cloud_sub_ = node_handle_.subscribe(cloud_topic_, 1, &Nodelet<PointT>::inputCloudCallback, this);
}

template<typename PointT>
void
patchworkpp::Nodelet<PointT>::inputCloudCallback(const sensor_msgs::PointCloud2::Ptr& cloud_msg)
{
  double time_taken;
  ...
  ROS_INFO_STREAM_THROTTLE(10.0,
                           "\033[1;32m"
                               ...
                               << "\033[0m");
  pubCloud(pub_cloud, pc_curr, cloud_msg->header.stamp, cloud_msg->header.frame_id);

  pubCloud(pub_ground, pc_ground, cloud_msg->header.stamp, cloud_msg->header.frame_id);
  pubCloud(pub_non_ground, pc_non_ground, cloud_msg->header.stamp, cloud_msg->header.frame_id);
}

template<typename PointT>
void
patchworkpp::Nodelet<PointT>::pubCloud(ros::Publisher pub,
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::initialize_zone(Zone& z, int num_sectors, int num_rings)
{
  z.clear();
  pcl::PointCloud<PointT> cloud;
  cloud.reserve(1000);
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::flush_patches_in_zone(Zone& patches, int num_sectors, int num_rings)
{
  for (int i = 0; i < num_sectors; i++)
  {
    for (int j = 0; j < num_rings; j++)
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::flush_patches(vector<Zone>& czm)
{
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::estimate_plane(const pcl::PointCloud<PointT>& ground)
{
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::extract_initial_seeds(const int zone_idx,
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::reflected_noise_removal(pcl::PointCloud<PointT>& cloud_in,
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::estimate_ground(pcl::PointCloud<PointT> cloud_in,
      ...
      for (int sector_idx = 0; sector_idx < num_sectors_each_zone_[zone_idx]; ++sector_idx)
      {
        ...
        if (visualize_)
        {
          ...
          set_ground_likelihood_estimation_status(
              zone_idx, ring_idx, concentric_idx, ground_uprightness, ground_elevation, ground_flatness);
        ...
        */
        ...
        else
        {
          RevertCandidate<PointT> candidate(
              concentric_idx, sector_idx, ground_flatness, line_variable, pc_mean_, regionwise_ground_);
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::update_elevation_thr()
{
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::update_flatness_thr()
{
  for (int i = 0; i < num_rings_of_interest_; i++)
  {
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::temporal_ground_revert(pcl::PointCloud<PointT>& cloud_ground,
                                                     pcl::PointCloud<PointT>& cloud_nonground,
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::extract_piecewiseground(const int zone_idx,
...
patchworkpp::Nodelet<PointT>::set_polygons(int zone_idx, int r_idx, int theta_idx, int num_split)
{
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::set_ground_likelihood_estimation_status(const int zone_idx,
                                                                      const int ring_idx,
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::calc_mean_stdev(vector<double> vec, double& mean, double& stdev)
{
  if (vec.size() <= 1)
    return;
...
template<typename PointT>
double
patchworkpp::Nodelet<PointT>::xy2theta(const double& x, const double& y)
{ // 0 ~ 2 * PI
  // if (y >= 0) {
...
template<typename PointT>
double
patchworkpp::Nodelet<PointT>::xy2radius(const double& x, const double& y)
{
...
template<typename PointT>
void
patchworkpp::Nodelet<PointT>::pc2czm(const pcl::PointCloud<PointT>& src,
                                     vector<Zone>& czm,
                                     pcl::PointCloud<PointT>& cloud_nonground)
{
...
{
using NodeletPointXYZI = patchworkpp::Nodelet<pcl::PointXYZI>;
}

PLUGINLIB_EXPORT_CLASS(patchworkpp::NodeletPointXYZI, nodelet::Nodelet);
